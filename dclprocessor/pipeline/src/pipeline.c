...
void *finalization_thread(void *arg){
    ...
    while(1){
        ...
        //fprintf(stdout, "[%s, %lx, %s]: clean data %p, data: %s\n", __FUNCTION__, module->thread_id, state_string(module->state), data, data);
        //fflush(stdout);
        data[0] = '\0';
...
void *initial_thread(void *arg){
    ...
    while(!(*(module->exit_flag))){
        ...
        strcpy(data, "test");
        //data[0] = '\0';
        fprintf(stdout, "[%s, %lx, %s]: initial data capture %p, data %s\n", __FUNCTION__, module->thread_id, state_string(module->state), data, data);
        fflush(stdout);
        ...
        fprintf(stdout, "[%s, %lx, %s]: next_module_idx = %d\n", __FUNCTION__, module->thread_id, state_string(module->state), module->next->idx);
        fflush(stdout);
...
void *source_thread(void *arg){
    struct pipeline_module *module = arg;
    struct pipeline_module_description *mdesc = module->mdesc;
    size_t dataoffset;
    char *data;
    int cmd;

    module->idx = process_start(mdesc->argv);
    if (module->idx < 0) {
        fprintf(stderr, "[%s, %lx, %s]: error on process %s start\n", __FUNCTION__, module->thread_id, mdesc->argv[0], state_string(module->state));
        fflush(stderr);
        *(module->exit_flag) = 1;
        sem_post(module->start_control);
        return NULL;
    ...
    module->proc_info = (struct process_info *)((char*)module->shmem + memory_get_processoffset(module->shmem)) + module->idx;
    fprintf(stdout, "[%s, %lx, %s]: %s is started\n", __FUNCTION__, module->thread_id, state_string(module->state), module->proc_info->process_name);
    fflush(stdout);
...
void *process_thread(void *arg){
    ...
    while (1) {
        ...
        data = (char*)module->shmem + dataoffset;
        fprintf(stdout, "[%s, %lx, %s]: current data %p, data: %s\n", mdesc->argv[0], module->thread_id, state_string(module->state), data, data);
        fflush(stdout);
        ...
        if (module->proc_info->cmd_result != 0){ //TODO: think about solution for this case
            ...
            fprintf(stdout, "[%s, %lx, %s]: processor status %d, skip data %p\n", mdesc->argv[0], module->thread_id, state_string(module->state), module->proc_info->cmd_result, data);
            fflush(stdout);
            // pushfree buffer
            ring_buffer_push(module->data_ring, dataoffset); 
            continue;
    ...
    fprintf(stdout, "[%s, %lx, %s]: stop\n", mdesc->argv[0], module->thread_id, state_string(module->state));
    fflush(stdout);
    sem_wait(&module->next->result);
    module->next->cmd = PROCESS_CMD_STOP;
    sem_post(&module->next->new_cmd);
