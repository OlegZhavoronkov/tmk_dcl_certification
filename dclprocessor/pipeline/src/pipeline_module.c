...
int pipeline_module_init(struct pipeline_module *module, void *shmem, struct ring_buffer *data_ring, int *exit_flag, sem_t *start_control) {
    if (!module) return -1;
    //ATTENTION: Don't set (module->id) here!
    module->idx = -200;
...
struct pipeline_module *pipeline_module_add(list_t *modules_list, int *modules_cnt, struct pipeline_module_description *mdesc){
    ...
    //out_pipeline_module_description(mdesc);

    module->id = mdesc->id;
    module->idx = -1; //thread with module is not started

    module->mdesc = mdesc; //TODO: think about copy structure
    if (mdesc->type == MT_FIRST) {
        if (mdesc->mode == MT_MODE_EXTERNAL) {
    ...
    else if (mdesc->type == MT_LAST) {
        module->thread_method = finalization_thread;
    }
    else if (mdesc->type == MT_MIDDLE){ //MT_MIDDLE
        module->thread_method = process_thread;
    }
    else {
        fprintf(stderr, "[%s]: unsupported type %d for module %d\n", __FUNCTION__, mdesc->type, mdesc->id);
        fflush(stderr);
    }
    ...
    //fprintf(stdout, "[%s]: control output for mdesc->id = %d\n", __FUNCTION__, mdesc->id);
    //fflush(stdout);
...
void pipeline_module_delete(list_t *modules_list, int *modules_cnt, struct pipeline_module *module){
    ...
    if (module->mdesc) module->mdesc = NULL; //when add set pointer to pdesc->mdescs[i]
    free(module);
}
...
int pipeline_modules_update(list_t *modules_list) {
    ...
    while(list_is_valid_elem(modules_list, item)){
        module = list_entry(item, struct pipeline_module, entry);
        module->next = pipeline_module_get(module->mdesc->next, modules_list);
        ...
        if (module->next == NULL) { //only one last module possible
            if (!null_found) {
                ...
                fprintf(stderr, "[%s]: second last (id = %d, next = %d)\n", __FUNCTION__, module->id, module->mdesc->next);
                fflush(stderr);
...
enum MD_STATE pipeline_module_set_state(enum MD_STATE state, struct pipeline_module *module) {
    ...
    module->state = state;
    fprintf(stdout, "[%s, id = %d]: state changed %s(%d) --> %s(%d)\n", __FUNCTION__, module->id, state_string(old_state), old_state, state_string(state), state);
    fflush(stdout);
    return old_state;
}
...
void pipeline_module_debug_out(struct pipeline_module *module) {
    ...
    fprintf(stdout, "\t{{id=%d}, {idx=%d},\n"
                    ...
                    state_string(module->state), module->state,
